// app/lib/audio.ts
// Convierte cualquier Blob de audio a WAV PCM16 mono @ MODEL_SR.
// Si ya es WAV PCM16 mono @ MODEL_SR, lo deja intacto.

export const MODEL_SR = 16000; // <-- cámbialo si tu modelo usa otra SR (p.ej. 16000)

const WAV_MIMES = new Set(["audio/wav", "audio/x-wav", "audio/wave"]);

export async function prepareAudioForUpload(fileOrBlob: Blob, targetSr = MODEL_SR): Promise<File> {
  // Si ya es WAV y cumple, no conviertas
  if (WAV_MIMES.has((fileOrBlob as File).type || "") && await isWavPcm16MonoAt(fileOrBlob, targetSr)) {
    return new File([fileOrBlob], "recording.wav", { type: "audio/wav" });
  }
  // Convierte a WAV PCM16 mono @ targetSr
  const wavBlob = await toWavPcm16Mono(fileOrBlob, targetSr);
...
export async function toWavPcm16Mono(blob: Blob, targetSampleRate = MODEL_SR): Promise<Blob> {
  ...
  if (targetSampleRate < 8000) {
    // Por si alguna vez usas SR raras: mejor enviar a 16000 y bajar en el server.
    throw new Error(`targetSampleRate=${targetSampleRate} < 8000 no soportado por OfflineAudioContext`);
  }
...
async function decodeAudio(ctx: AudioContext, arrayBuf: ArrayBuffer): Promise<AudioBuffer> {
  // Safari a veces falla con promise directa; hacemos fallback
